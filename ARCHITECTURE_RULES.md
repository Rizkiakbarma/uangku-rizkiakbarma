...
- **Database Utama (Baru):** Supabase (PostgreSQL).
- **Database Warisan (Lama):** Google Sheets via Google Apps Script (GAS). API masih aktif berstatus hibrida.
- **I/O Node:** Telegram Bot API.

## 3. Aturan Manajemen State & Fetching (CRITICAL)
- **State Management:** Semua state diatur secara lokal menggunakan `AppContext.tsx` dan Custom Hook `useFinanceData.ts`. Kami meminimalisir penggunaan pustaka eksternal raksasa seperti Redux.
- **Global Caching:** `useFinanceData.ts` memiliki variabel *singleton* lokal (`globalHasFetchedData` dll) untuk mencegah re-render berulang lintas navigasi perutean (Router Navigation). Anda dilarang mematikan pelindung *cache global* ini!
- **Data Jumping BUG (STRICTLY PROHIBITED):** DILARANG MERANCANG SISTEM *PROGRESSIVE LOADING*! Proses sinkronisasi data dari Supabase dan GAS HARUS menunggu secara statis menggunakan paradigma `Promise.allSettled`. Data baru boleh dirender jika proses keduanya 100% tuntas untuk menghindari angka pada grafik melompat.
