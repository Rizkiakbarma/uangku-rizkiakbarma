...
import { 
  ...
  ArrowUpRight, ArrowDownRight, Zap, BarChart3, LineChart
} from 'lucide-react';
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2rem] p-8 max-w-sm w-full shadow-2xl border border-slate-100">
            ...
            <div className="flex gap-3 mt-8">
              ...
              <button onClick={handleDelete} className="flex-1 py-4 bg-rose-600 text-white rounded-2xl font-bold shadow-lg">{isDeleting ? "..." : "Ya, Hapus"}</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
            ...
            <div className="flex items-center gap-3 bg-slate-50 p-3 rounded-2xl border border-slate-100">
              <div className="text-right"><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Target Anda</p>
                {isSettingBudget ? <input type="number" value={monthlyBudget} onChange={(e) => setMonthlyBudget(parseInt(e.target.value))} className="w-28 bg-transparent text-sm font-black text-emerald-700 outline-none border-b-2 border-emerald-500" autoFocus /> : <p className="text-sm font-black text-slate-800">{formatRp(monthlyBudget)}</p>}
              </div>
          ...
          <div className="bg-white rounded-[2.5rem] p-10 shadow-sm border border-slate-100 lg:col-span-2 relative transition-all hover:border-emerald-100">
            ...
            <div className="h-72 flex items-end justify-between gap-4 pb-4 relative">
              {isBarChart ? last7DaysData.map((data, idx) => {
                ...
                return (<div key={idx} className="flex flex-col items-center flex-1 group relative h-full justify-end"><span className="text-[9px] font-black text-rose-600 mb-2 whitespace-nowrap">{data.amount > 0 ? formatShortRp(data.amount) : ''}</span><div className="w-full max-w-[45px] bg-rose-600 rounded-2xl transition-all duration-700 shadow-md" style={{ height: `${Math.max(heightPct, 5)}%` }}></div><span className="text-[9px] text-slate-400 mt-4 font-black uppercase tracking-tighter text-center">{data.label}</span></div>);
              }) : (
                <div className="w-full h-full relative"><svg viewBox="0 0 700 250" className="w-full h-full overflow-visible"><path d={`M 0 250 ${last7DaysData.map((d, i) => `L ${i * 116} ${250 - (d.amount / maxDaily * 180)}`).join(' ')} L 696 250 Z`} fill="#fecdd3" opacity="0.4" /><path d={last7DaysData.map((d, i) => `${i === 0 ? 'M' : 'L'} ${i * 116} ${250 - (d.amount / maxDaily * 180)}`).join(' ')} fill="none" stroke="#e11d48" strokeWidth="6" strokeLinecap="round" strokeLinejoin="round" />
                    {last7DaysData.map((d, i) => (<g key={i}><circle cx={i * 116} cy={250 - (d.amount / maxDaily * 180)} r="7" fill="#fff" stroke="#e11d48" strokeWidth="4" />{d.amount > 0 && <text x={i * 116} y={250 - (d.amount / maxDaily * 180) - 15} textAnchor="middle" className="text-[11px] font-black fill-rose-600">{formatShortRp(d.amount)}</text>}</g>))}
                  </svg>
