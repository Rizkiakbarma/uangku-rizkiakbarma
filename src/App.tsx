...
export default function App() {
  ...
  // 🔗 PASTI KAN INI ADALAH URL WEB APP GOOGLE SCRIPT KAMU
  ...
  const filteredByMonth = useMemo(() => {
    return transactions.filter(t => 
      t.month === viewDate.getMonth() && t.year === viewDate.getFullYear()
    );
  ...
  const sisaSaldo = useMemo(() => {
    const masuk = transactions.filter(t => t.type?.toLowerCase() === 'masuk').reduce((acc, curr) => acc + (Number(curr.amount) || 0), 0);
    const keluar = transactions.filter(t => t.type?.toLowerCase() === 'keluar').reduce((acc, curr) => acc + (Number(curr.amount) || 0), 0);
    return masuk - keluar;
  }, [transactions]);
  ...
  if (error) return <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 font-sans"><div className="bg-white p-10 rounded-[2.5rem] shadow-xl border border-red-50 max-w-md w-full text-center"><Lock className="w-16 h-16 text-red-500 mx-auto mb-6" /><h2 className="text-2xl font-black mb-2 text-slate-900">Akses Terkunci</h2><p className="text-slate-500 text-sm mb-8 leading-relaxed">{error}</p></div></div>;

  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 pb-20 text-[14px]">
      {deleteId && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2rem] p-8 max-w-sm w-full shadow-2xl border border-slate-100">
            ...
            <h3 className="text-xl font-black text-center mb-2">Hapus?</h3>
            <div className="flex gap-3 mt-8">
              <button onClick={() => setDeleteId(null)} className="flex-1 py-4 bg-slate-100 rounded-2xl font-bold">Batal</button>
              <button onClick={handleDelete} className="flex-1 py-4 bg-rose-600 text-white rounded-2xl font-bold">{isDeleting ? "..." : "Ya"}</button>
            </div>
          </div>
        </div>
      )}
      ...
      <nav className="bg-emerald-900 text-white px-6 py-6 shadow-2xl sticky top-0 z-50">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex items-center gap-4">
            ...
            <div><h1 className="text-xl font-black tracking-tighter uppercase leading-none">UANGKU PRO</h1><p className="text-emerald-400 text-[9px] font-black uppercase mt-1 italic">By Rizkiakbarma</p></div>
          </div>
          <div className="flex items-center bg-emerald-800/50 p-1.5 rounded-2xl border border-emerald-700">
            <button onClick={() => changeMonth(-1)} className="p-2 hover:bg-emerald-700 rounded-xl transition-colors"><ChevronLeft className="w-5 h-5" /></button>
            <div className="px-6 text-center min-w-[140px]"><p className="text-[10px] font-black uppercase text-emerald-400">Periode</p><p className="text-sm font-black uppercase">{viewDate.toLocaleDateString('id-ID', { month: 'long', year: 'numeric' })}</p></div>
            <button onClick={() => changeMonth(1)} className="p-2 hover:bg-emerald-700 rounded-xl transition-colors"><ChevronRight className="w-5 h-5" /></button>
          </div>
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-10">
          <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100 relative overflow-hidden group"><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Total Saldo</p><h2 className="text-4xl font-black text-emerald-900 tracking-tight">{formatRp(sisaSaldo)}</h2><div className="mt-6 flex items-center gap-2 text-emerald-600 bg-emerald-50 w-fit px-4 py-1.5 rounded-full text-[10px] font-black border border-emerald-100"><CheckCircle2 className="w-3.5 h-3.5" /> VERIFIED</div></div>
          <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100 flex flex-col justify-center transition-all hover:border-blue-100"><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 text-blue-600">Inflow {viewDate.toLocaleDateString('id-ID', { month: 'short' })}</p><h2 className="text-2xl font-black text-blue-700">{formatRp(totalMasukBulanIni)}</h2></div>
        ...
        <div className="bg-white rounded-[2.5rem] p-10 shadow-sm border border-slate-100 mb-10 relative overflow-hidden transition-all hover:border-emerald-200">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
            ...
            <div className="flex items-center gap-3 bg-slate-50 p-3 rounded-2xl border border-slate-100"><div className="text-right"><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Target Anda</p>{isSettingBudget ? <input type="number" value={monthlyBudget} onChange={(e) => setMonthlyBudget(parseInt(e.target.value))} className="w-28 bg-transparent text-sm font-black text-emerald-700 outline-none border-b-2 border-emerald-500 font-sans" autoFocus /> : <p className="text-sm font-black text-slate-800">{formatRp(monthlyBudget)}</p>}</div><button onClick={() => isSettingBudget ? handleSaveBudget() : setIsSettingBudget(true)} className="p-2.5 bg-white rounded-xl shadow-sm text-emerald-600 hover:text-emerald-700 transition-all hover:scale-110">{isSettingBudget ? <CheckCircle2 className="w-5 h-5" /> : <Filter className="w-5 h-5" />}</button></div>
          </div>
        ...
        <footer className="mt-24 flex flex-col md:flex-row justify-between items-center gap-8 px-6 border-t border-slate-200 pt-12 text-slate-400">
          <div><p className="text-[10px] font-black uppercase tracking-[0.2em]">Secured by UangKu Rizkiakbarma</p><p className="text-[9px] font-bold mt-1.5 uppercase tracking-tight text-slate-300">Sistem Manajemen Keuangan v8.6 PRO Stable</p></div>
          <span className="text-[10px] font-black uppercase tracking-widest bg-slate-100 px-5 py-2.5 rounded-2xl border border-slate-200/50">Production Ready Build</span>
        </footer>
      </main>
